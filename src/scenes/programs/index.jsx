...
const Programs = () => {
  ...
  const handleOpenModal = () => {
    setIsEditMode(false);
    setIsModalOpen(true);
    if (!isEditMode) { // Solo generar un nuevo ID si se está agregando un nuevo programa
  ...
  const handleSaveData = () => {
    if (
      ...
      if (programIndex !== -1) {
        ...
        if (!isEditMode) { // Solo agregar un nuevo programa si no estamos en modo de edición
